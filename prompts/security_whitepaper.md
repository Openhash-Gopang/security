...
**원칙 2 — 독자 인증 구현 금지**
K-Security는 자체 인증 시스템을 운영하지 않습니다. 모든 인증은 `hondi.net`이 전담합니다. K-Security는 결과만 수신합니다.
...
| K-Security | `security` | security.hondi.net | 30초 보고 주기 유지 |
| gopang-proxy | `gopang-proxy` | tensor-city.workers.dev | `/auth` `/pdv/report` `/deepseek` 응답 <300ms |

#### 안전·사법
| 시스템 | ID | 도메인 | 정상 행동 기준 |
|--------|-----|--------|---------------|
| K-Police | `police` | police.hondi.net | 신고→PDV→출동번호 흐름 정상 |
| K-119 | `911` | 911.hondi.net | 응급 분류 1~4급, K-Police 연동 정상 |
| K-Law | `klaw` | klaw.hondi.net | 30초 쿨다운 모니터, 판결 정확도 9.99/10 |

#### 경제·금융
| 시스템 | ID | 도메인 | 정상 행동 기준 |
|--------|-----|--------|---------------|
| GDC | `gdc` | gdc.hondi.net | BIVM **Σδ=0** 항등식 유지, 이중지불 없음 |
| K-Tax | `tax` | tax.hondi.net | 거래 발생 즉시 세금 분리, 실시간 정산 |
| K-Stock | `stock` | stock.hondi.net | 89개 자산군 갱신, 이상 거래 없음 |
| K-Insurance | `insurance` | insurance.hondi.net | 유효성 점수 >0.9 유지 |
| K-Market | `market` | market.hondi.net | 주문-결제-PDV 흐름 정상 |

#### 교통·물류
| 시스템 | ID | 도메인 | 정상 행동 기준 |
|--------|-----|--------|---------------|
| K-Traffic | `traffic` | traffic.hondi.net | 이동 수단 조합 응답 <1s |
| K-Logistics | `logistics` | logistics.hondi.net | 주문-출고-배송 흐름, K-Traffic 연동 |

#### 의료·교육
| 시스템 | ID | 도메인 | 정상 행동 기준 |
|--------|-----|--------|---------------|
| K-Health | `health` | health.hondi.net | AI 주치의 응답 정상, K-119 연동 |
| K-School | `school` | school.hondi.net | 세션 평균 12분, PDV 학습 기록 |

#### 공공·거버넌스
| 시스템 | ID | 도메인 | 정상 행동 기준 |
|--------|-----|--------|---------------|
| K-Public | `public` | public.hondi.net | 228개 기관 서비스, 증명서 발급 |
| K-Democracy | `democracy` | democracy.hondi.net | DAWN 투표 무결성, 이중 투표 없음 |
